...
幕基底，并增加了逐方法输入预处理冒烟测试。

| 制品 | 位置 |
|------|------|
| 数据集（OOPAO 屏幕） | `data/beaconless_demo.h5`（2 500 个样本，3.93 GB） |
...
| 训练运行 | `sparkling-plasma-1`（`1jjq96ae`） |
| 评估运行 | `eoudxg1u` |
| 预处理冒烟运行 | `094tewxf` |
...
检查点：`checkpoints/best.pt`（仿真 FOM 最佳），`checkpoints/last.pt`。

---

## 4. 评估（第 2.7 节）

`evaluate.py` 在 100 样本评估集上运行，OOPAO 数据集（`eoudxg1u`）：
...
ML 分支（FOM_ML 0.486）优于 noao/track 基线（0.282）→ 增益 1.72，η 0.34。
...
系数 RMS 误差以及物理仿真 FOM_ML。结果已上传至 WandB（`094tewxf`）。

### 5.1 方法与结果

| # | 方法 | 输入均值 | 输入最大值 | 特征最大值 | 预测 RMS（rad） | R_j 均值 | **FOM_ML** |
|---|--------|-----------|-----------|----------|----------------|---------|-----------|
| 0 | **baseline_norm**（3 平面，/2047） | 0.043 | 0.93 | 6.65 | 0.161 | 0.163 | **0.506** |
| 1 | raw_uint16（3 平面，0–2047） | 88.6 | 1899 | 22 258 | 734.9 | 0.031 | 0.0007 |
| 2 | minmax_sample（3 平面，逐样本） | 0.071 | 1.0 | 13.1 | 0.294 | 0.086 | 0.176 |
| 3 | minmax_global（3 平面，全局） | 0.047 | 1.0 | 7.39 | 0.168 | 0.148 | 0.481 |
| 4 | zscore（3 平面，逐集合 z-score） | ~0 | 18.7 | 163.6 | 4.12 | 0.024 | 0.0011 |
| 5 | oneplane_norm（1 焦平面，/2047） | 0.015 | 0.93 | 10.68 | 0.225 | 0.040 | 0.362 |
| 6 | oneplane_raw（1 焦平面，0–2047） | 31.5 | 1899 | 28 159 | 446.9 | −0.003 | 0.0008 |

### 5.2 发现

1. **尺度是一个硬约束。** 任何使像素值偏离 `/2047` 尺度（raw_uint16，
   zscore，oneplane_raw）的方法都会破坏网络：特征图饱和（特征最大值 163–28 159
   对比基线的 ~7）且系数误差急剧增大（RMS 4–735 rad vs 0.16）。FOM_ML
   崩溃至 ~0。BatchNorm 层部分吸收了尺度，但 MLP 头部和下游去归一化
   （`c = y·σ + μ`）是针对 `/2047` 范围校准的，无法从 2047× 或 z 标准化的输入中
   恢复。

2. **全局 min-max ≈ 基线。** minmax_global（FOM 0.481）与基线（0.506）
   非常接近，因为评估集的动态范围几乎匹配 12-bit 标度——该变换接近恒等映射。
   逐样本 min-max（FOM 0.176）则差得多：它将每个样本重新归一化到完整的 0–1，
   抹去了网络所学习的样本之间相对强度差异。

3. **单平面退化但不会崩溃。** oneplane_norm（FOM 0.362）保留了合理的特征
   （特征最大值 ~11）并保留了部分校正能力，但相比基线损失了 ~28% 的 FOM。
   3 通道设计编码了用于深度感知相位恢复的 ±z_R 离焦视差；使用单平面时网络
   退化为 2D 相位估计，高阶 Zernike 模式精度下降（R_j 0.040 vs 0.163）。

4. **复合故障模式是灾难性的。** oneplane_raw（1 平面 + 错误尺度）是
   raw_uint16 之后最差的情况（FOM 0.0008，R_j ≈ 0）——两种违规叠加。
...
### 7.1 训练运行（`sparkling-plasma-1`，`1jjq96ae`）
...
- `sim/median_fom_ml` 逐步上升并最终稳定在 0.47 左右 → 仿真环境下的校正
  能力在提升；当 `sim/gain > 1` 时表示 ML 校正已超越纯跟踪。
- `sim/eta` 反映 ML 距离理论上界（Z78）还有多远；本实验最终 η ≈ 0.34，
  表明仍有提升空间。
...
| 指标名称 | 含义 |
|---------|------|
| `gain` | 最终增益 `g = median(FOM_ML) / median(FOM_track)` = **1.721**。ML 校正使 FOM 达到跟踪基准的 1.72 倍。 |
| `eta` | 最终效能 `η = (median(FOM_ML) - median(FOM_track)) / (median(FOM_Z78) - median(FOM_track))` = **0.342**。ML 实现了理想上界与跟踪基准之间差距的 34.2%。 |
| `Rj_mean` | 78 个 Zernike 模式 Pearson 相关系数 `R_j`（公式 17）的均值 = **0.171**。反映整体预测精度。 |
...
| `median_fom/ml` | **CNN 预测相位**的中位 FOM = **0.486**。 |
...
| 指标名称 | 含义 |
|---------|------|
| `{method}/input_mean` | 输入图像所有像素的均值。用于快速判断输入是否处于训练时的 `[0, 1]` 标度（基线均值 ~0.04）。 |
| `{method}/input_std` | 输入图像所有像素的标准差。 |
| `{method}/input_max` | 输入图像的最大像素值。基线约 0.93（接近 1.0）；raw_uint16 可达 1899（未归一化）。 |
...
| `{method}/feat_max_mean` | 128 个特征图最大值的均值。**与 `feat_mean_abs` 共同诊断饱和**：基线约 6.65，raw_uint16 可达 22 258（严重饱和）。 |
| `{method}/pred_rms` | 预测系数与真实系数之间的 RMS 误差（弧度）。基线约 0.16 rad；raw_uint16 达 734.9 rad（完全失效）。 |
| `{method}/rj_mean` | 78 个模式 `R_j` 的均值。基线约 0.16；raw_uint16 约 0.03（几乎无预测能力）。 |
| `{method}/rj_std` | 78 个模式 `R_j` 的标准差。 |
| `{method}/fom_ml_median` | 该预处理方法在物理仿真下的中位 FOM_ML。**最终性能指标**：基线 0.506，raw_uint16 约 0.0007。 |
