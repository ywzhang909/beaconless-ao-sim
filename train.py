...
class BeaconlessH5Dataset(torch.utils.data.Dataset):
        ...
        *,
        ...
        if self.preload:
            with h5py.File(h5_path, "r") as f:
                ...
                if "/L" in f:
                    self._lengths = f["/L"][idx].astype(np.float32)  # (n,) float32
    ...
    def __getitem__(self, i: int) -> dict[str, torch.Tensor]:
        ...
        if self.preload:
            length = self._lengths[i] if self._lengths is not None else np.float32(1000.0)
        else:
            length = np.float32(f["/L"][idx]) if "/L" in f else np.float32(1000.0)
        return {
