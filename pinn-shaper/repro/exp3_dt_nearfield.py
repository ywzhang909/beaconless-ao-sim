"""
Experiment 3: DT-letters-shaped flat-top (near field)
======================================================
Faithful reproduction of "DT-letters-shaped flat-top (near field).ipynb":
  - Gaussian source (w_I=1mm) over 6mm aperture, lambda=1um
  - DT-letters target over 10mm target plane at z=10cm (scale_factor=2 propagation)
  - Nearfield_PINN: 4 hidden layers x 150 neurons, tanh, phi_scale=30
  - Training: Adam(1000x1000) -> Adam(2000x15000) -> LBFGS(22500, 4000 it) -> LBFGS(22500, 1000 it)
  - Validation: scalar angular-spectrum propagation (2048x2048), scale_factor=2
"""
import os
...
E_fun = create_interpolator(torch.tensor(image), mode="bilinear",
                            region=[-extent_target / 2, extent_target / 2, -extent_target / 2, extent_target / 2],
                            align_corners=True)

lam = 1.0 * um
z = 10 * cm
target_plane_scale_factor = 2.0   # enlarged output window (DT notebook)

solver = Nearfield_PINN(I_fun, E_fun, extent_source, extent_target, z, lam,
                        integration_points=5000, hidden_layers=4,
...
if not args.no_train:
    print("=" * 70)
    print("Experiment 3 (DT near-field): FULL TRAINING FROM SCRATCH")
    print("=" * 70)
    ...
    torch.save(solver.net.state_dict(), "reproduced_models/DT_flat-top-nearfield_model.pth")
    print("Saved model -> reproduced_models/DT_flat-top-nearfield_model.pth")
else:
    print("Loading reproduced model (no training)...")
    solver.net.load_state_dict(torch.load("reproduced_models/DT_flat-top-nearfield_model.pth"))
...
fig.tight_layout()
fig.savefig("results/exp3_dt_nearfield_phase.png", dpi=150)
print("Saved results/exp3_dt_nearfield_phase.png")

# ------------------------------------------------------------------
# Angular-spectrum validation (notebook cell 17)
# ------------------------------------------------------------------
import diffractsim
...
F = MonochromaticField(wavelength=lam, extent_x=extent_source, extent_y=extent_source, Nx=2048, Ny=2048)
F.E = np.sqrt(I_fun_numpy(F.xx, F.yy)) * np.exp(1j * phi_spline(F.x, F.y))
F.propagate(z, scale_factor=target_plane_scale_factor)

# ------------------------------------------------------------------
# Comparison plot (notebook cell 19)
# ------------------------------------------------------------------
...
ax1.set_xlabel("$x$ [mm]"); ax1.set_ylabel("$y$ [mm]")
ax1.set_title("Simulated profile at the target plane"); ax1.set_aspect("equal")
...
ax2.legend()
fig.tight_layout()
fig.savefig("results/exp3_dt_nearfield_propagation.png", dpi=150)
print("Saved results/exp3_dt_nearfield_propagation.png")
...
mask = np.abs(x_torch.cpu().numpy()) / mm <= 5.0   # within target window [-5,5]mm
...
print(f"\n[Exp3 metrics, y=0 cross-section]")
